...
def fenci(training_data):
    ...
    training_data="".join(pattern.findall(training_data))
...
def engprocess(training_data):
    training_data.lower()
    training_data= re.sub(r"([?.!,¿])", r" \1 ", training_data)
    training_data = re.sub(r'[" "]+', " ", training_data)
    training_data=re.sub(r"[^a-zA-Z?.!,¿]+", " ", training_data)
    training_ci = training_data.strip().split()
    return training_ci
...
def create_vocabulary(vocabulary_file, raw_data_dir, max_vocabulary_size, Isch=True, normalize_digits=False):
...
def sentence_to_ids(sentence, vocabulary,
                    normalize_digits=False, Isch=True):
    if normalize_digits:
        ...
        with gfile.GFile(data_file_name, mode="rb") as data_file:
            ...
            with gfile.GFile(target_file_name, mode="w") as ids_file:
                ...
                for line in data_file:
                    ...
                    token_ids = sentence_to_ids(line.decode('UTF-8'), vocab, normalize_digits, Isch)
                    ids_file.write(" ".join([str(tok) for tok in token_ids]) + "\n")


# 将文件批量转成ids文件
def textdir_to_idsdir(textdir, idsdir, vocab, normalize_digits=False, Isch=True):
...
def main():
    ...
    textdir_to_idsdir(raw_data_dir_to, data_dir + "toids/", vocaben, normalize_digits=False, Isch=False)
    textdir_to_idsdir(raw_data_dir, data_dir + "fromids/", vocabch, normalize_digits=False, Isch=True)
